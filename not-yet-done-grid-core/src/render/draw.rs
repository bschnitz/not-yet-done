...
pub fn draw_borders<T: RenderTarget>(
    cfg: &GridConfig,
    layout: &GridLayout,
    target: &mut T,
) {
...
fn draw_outer_frame<T: RenderTarget>(
    cfg: &GridConfig,
    layout: &GridLayout,
    target: &mut T,
) {
...
fn draw_horizontal_lines<T: RenderTarget>(
    cfg: &GridConfig,
    layout: &GridLayout,
    target: &mut T,
) {
    ...
    for (gap_idx, slot) in cfg.h_gaps.iter().enumerate() {
        ...
        let Some(&y)   = layout.h_gap_y.get(gap_idx).and_then(|v| v.as_ref()) else { continue };

        let outer_same = cfg.outer_border.map(|ob| same_style(ob, bc)).unwrap_or(false);
        let active_xs = h_active_xs(cfg, layout, gap_idx, 0, cfg.cols - 1);

        for &x in &active_xs { target.put_char(x, y, bc.horizontal); }
...
fn draw_h_spanned<T: RenderTarget>(
    ...
    let Some(&y) = layout.h_gap_y.get(span.gap_index).and_then(|v| v.as_ref()) else { return };
...
fn h_active_xs(
    ...
    for col in col_start..=col_end {
        if cfg.is_inside_v_group(h_gap_idx, col) { continue; }
        let x0 = layout.col_x[col];
        let x1 = x0 + layout.col_w[col];
        for x in x0..x1 { xs.push(x); }
    }
    // v-gap columns within the range.
    for gap_col in col_start..col_end {
        let Some(&gx) = layout.v_gap_x.get(gap_col).and_then(|v| v.as_ref()) else { continue };
...
fn draw_vertical_lines<T: RenderTarget>(
    cfg: &GridConfig,
    layout: &GridLayout,
    target: &mut T,
) {
    ...
    for (gap_idx, slot) in cfg.v_gaps.iter().enumerate() {
        let Some(slot) = slot else { continue };
        let Some(bc)   = slot.border else { continue };
        let Some(&x)   = layout.v_gap_x.get(gap_idx).and_then(|v| v.as_ref()) else { continue };

        let outer_same = cfg.outer_border.map(|ob| same_style(ob, bc)).unwrap_or(false);
        let active_ys = v_active_ys(cfg, layout, gap_idx, 0, cfg.rows - 1);

        for &y in &active_ys { target.put_char(x, y, bc.vertical); }
...
fn draw_v_spanned<T: RenderTarget>(
    ...
    let Some(&x) = layout.v_gap_x.get(span.gap_index).and_then(|v| v.as_ref()) else { return };
...
fn v_active_ys(
    ...
    for row in row_start..=row_end {
        if cfg.is_inside_h_group(row, v_gap_idx) { continue; }
        let y0 = layout.row_y[row];
        let y1 = y0 + layout.row_h[row];
        for y in y0..y1 { ys.push(y); }
    }
    // h-gap rows within the range.
    for gap_row in row_start..row_end {
        let Some(&gy) = layout.h_gap_y.get(gap_row).and_then(|v| v.as_ref()) else { continue };
...
fn draw_crossings<T: RenderTarget>(
    cfg: &GridConfig,
    layout: &GridLayout,
    target: &mut T,
) {
    ...
    for (vi, vx) in layout.v_gap_x.iter().enumerate() {
        ...
        for (hi, hy) in layout.h_gap_y.iter().enumerate() {
            ...
            let h_right = h_bc.is_some()
                && vi + 1 < cfg.cols && !cfg.is_inside_v_group(hi, vi + 1);
            let v_above = v_bc.is_some() && !cfg.is_inside_h_group(hi, vi);
            let v_below = v_bc.is_some()
                && hi + 1 < cfg.rows && !cfg.is_inside_h_group(hi + 1, vi);
    ...
    for span in &cfg.h_spanned {
        let Some(&y) = layout.h_gap_y.get(span.gap_index).and_then(|v| v.as_ref()) else { continue };
        ...
        for (vi, vx) in layout.v_gap_x.iter().enumerate() {
            ...
            apply_crossing(x, y, span.border, v_bc, h_left, h_right, v_above, v_below, target);
        }
    }

    // Spanned V × Full H crossings.
    for span in &cfg.v_spanned {
        let Some(&x) = layout.v_gap_x.get(span.gap_index).and_then(|v| v.as_ref()) else { continue };
        ...
        for (hi, hy) in layout.h_gap_y.iter().enumerate() {
            ...
            apply_crossing(x, y, h_bc, span.border, h_left, h_right, v_above, v_below, target);
        }
    }

    // Spanned V × Spanned H crossings.
    for h_span in &cfg.h_spanned {
        let Some(&y) = layout.h_gap_y.get(h_span.gap_index).and_then(|v| v.as_ref()) else { continue };
        ...
        for v_span in &cfg.v_spanned {
            let Some(&x) = layout.v_gap_x.get(v_span.gap_index).and_then(|v| v.as_ref()) else { continue };
            if x < hx_start || x > hx_end { continue; }
            let vy_start = layout.row_y[v_span.start];
            let vy_end   = layout.row_y[v_span.end] + layout.row_h[v_span.end] - 1;
            if y < vy_start || y > vy_end { continue; }
            ...
            apply_crossing(x, y, h_span.border, v_span.border,
                h_left, h_right, v_above, v_below, target);
        }
    }
...
fn apply_crossing<T: RenderTarget>(
    ...
    let (Some(h), Some(v)) = (h_bc, v_bc) else { return };
    if !same_style(h, v) { return; }
...
fn draw_border_texts<T: RenderTarget>(
    cfg: &GridConfig,
    layout: &GridLayout,
    target: &mut T,
) {
...
fn write_h_text<T: RenderTarget>(
    ...
    if truncate {
        ...
        if x_ell <= x_max { target.put_char(x_ell, y, '…'); }
    }
...
fn write_v_text<T: RenderTarget>(
    ...
    if truncate {
        ...
        if y_ell <= y_max { target.put_char(x, y_ell, '…'); }
    }
}
...
fn content_width(layout: &GridLayout) -> usize {
    if layout.col_x.is_empty() { return 0; }
...
fn content_height(layout: &GridLayout) -> usize {
    if layout.row_y.is_empty() { return 0; }
