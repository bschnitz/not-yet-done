    ...
    pub fn set_border_text(
        ...
        match pos {
            ...
            BorderPos::AfterRow(i) => {
                self.set_h_gap(*i);
                if let Some(Some(slot)) = self.h_gaps.get_mut(*i) {
                    slot.text = Some(entry);
                }
            }
            BorderPos::BeforeRow(i) => {
                self.set_h_gap(i - 1);
                if let Some(Some(slot)) = self.h_gaps.get_mut(i - 1) {
                    slot.text = Some(entry);
                }
            }
            BorderPos::AfterCol(i) => {
                self.set_v_gap(*i);
                if let Some(Some(slot)) = self.v_gaps.get_mut(*i) {
                    slot.text = Some(entry);
                }
            }
            BorderPos::BeforeCol(i) => {
                self.set_v_gap(i - 1);
                if let Some(Some(slot)) = self.v_gaps.get_mut(i - 1) {
                    slot.text = Some(entry);
                }
            }
            BorderPos::AfterRowSpanned {
