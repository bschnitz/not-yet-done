//! sea-orm-backed [`SessionStore`] for the Taiga adapter.
//!
//! Persists the orchestrator's opaque session blob — adapter-side JSON
//! containing the JWT pair plus user identity — in the `auth_session`
//! table, keyed by the connection's `scope_id` (UUID v5 of the base URL).
...
mod tests {
    ...
    async fn roundtrip_save_load_delete() {
        ...
        let entry = SessionEntry {
            blob: r#"{"auth_token":"synthetic"}"#.into(),
