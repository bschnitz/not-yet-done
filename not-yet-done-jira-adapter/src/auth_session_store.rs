//! sea-orm-backed [`SessionStore`] for the Jira adapter.
//!
//! Persists the orchestrator's opaque session blob — adapter-side JSON
//! containing the cookie (cookie mechanism) or email + token pair
//! (basic-auth mechanism) — in the `auth_session` table, keyed by the
//! connection's `scope_id` (UUID v5 of the base URL).
...
mod tests {
    ...
    async fn separate_scopes_dont_collide() {
        ...
        let store_a = SqlAuthSessionStore::new(db.clone(), scope_a);
        let store_b = SqlAuthSessionStore::new(db.clone(), scope_b);
        ...
        assert_eq!(store_a.load().await.unwrap().blob, "a-blob");
        assert_eq!(store_b.load().await.unwrap().blob, "b-blob");

        store_a.delete().await;
