//! Proc-macro crate for the `not-yet-done` workspace.
//!
//! Exports:
//! - [`macro@ColumnRegistry`] — SeaORM column registry for `not-yet-done-core`.
//! - [`macro@AllVariants`] — inherent `const ALL: &[Self]` for fieldless enums,
//!   used crate-wide for compile-time-complete variant iteration.
//!
...
/// Derive an inherent `pub const ALL: &'static [Self]` listing every variant,
/// in declaration order.
///
/// Only valid for **fieldless** enums (all variants must be unit variants) —
/// the point is compile-time completeness for enums used as keys/dispatch
/// discriminators. Adding a variant automatically extends `ALL`, so code that
/// iterates over it (e.g. bar builders that place every action) can never
/// silently drop the new variant.
///
/// ```rust,ignore
/// #[derive(AllVariants)]
/// enum Direction { North, South }
/// // generates: impl Direction { pub const ALL: &'static [Self] = &[Self::North, Self::South]; }
/// ```
#[proc_macro_derive(AllVariants)]
pub fn derive_all_variants(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    match impl_all_variants(input) {
        Ok(ts) => ts.into(),
        Err(e) => e.to_compile_error().into(),
    }
}

fn impl_all_variants(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    let data = match &input.data {
        Data::Enum(e) => e,
        _ => {
            return Err(syn::Error::new(
                Span::call_site(),
                "AllVariants can only be derived for enums",
            ));
        }
    };
    ...
    for v in &data.variants {
        if !matches!(v.fields, Fields::Unit) {
            return Err(syn::Error::new_spanned(
                &v.ident,
                "AllVariants requires every variant to be fieldless (unit)",
            ));
        }
    ...
    Ok(quote! {
        impl #impl_generics #name #ty_generics #where_clause {
            ...
            /// Auto-generated by `#[derive(AllVariants)]`.
...
fn registry_ident(input: &DeriveInput) -> syn::Result<Ident> {
    ...
    let base = struct_name.strip_suffix("Model").unwrap_or(&struct_name);
    Ok(Ident::new(
        &format!("{base}ColumnRegistry"),
        Span::call_site(),
    ))
}
