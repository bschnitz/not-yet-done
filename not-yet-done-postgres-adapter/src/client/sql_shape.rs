...
//! actually parsing it. Promoted out of `adapter/mod.rs` so the
//! cursor path can share the same multi-statement detector and
//! SELECT/WITH sniffer.
//!
...
mod tests {
    ...
    fn recognizes_select_and_with() {
        ...
        assert!(looks_like_select_or_with("select 1"));
        assert!(looks_like_select_or_with("WITH x AS (SELECT 1) SELECT * FROM x"));
        assert!(looks_like_select_or_with("with x as (select 1) select * from x"));
    }
    ...
    fn leading_comment_and_whitespace_still_detects_select() {
        assert!(looks_like_select_or_with("-- explain\n  /* why */  SELECT 1"));
    }
    ...
    fn multi_statement_ignores_semicolon_in_block_comment() {
        assert!(!has_multiple_statements("SELECT 1 /* split ; here */ FROM t"));
    }
    ...
    fn split_trailing_three_statements_takes_last() {
        let (pre, last) =
            split_trailing_statement("SET a = 1; SET b = 2; SELECT a + b").unwrap();
