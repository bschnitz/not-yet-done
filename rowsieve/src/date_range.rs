...
//!
//! A host that fetches rows from somewhere expensive — an API, a calendar
//! server — usually has to ask for a *window* before it can filter anything.
//! This derives the tightest window a filter can possibly match, from the
//! comparisons it makes on the fields the caller names as dates.
//!
//! The answer is always a superset: it is a hint for narrowing a fetch, never a
//! substitute for evaluating the filter afterwards.
//!
...
/// The tightest `(min, max)` window `expr` can match.
///
/// `date_fields` names the fields to read as dates; comparisons on anything
/// else say nothing about the window and leave it unbounded. Which fields
/// those are is the host's knowledge — this crate has no schema to ask.
pub fn extract_date_bounds(expr: &FilterExpr, date_fields: &[&str]) -> DateBounds {
    match expr {
        FilterExpr::And(children) => {
            let mut result = DateBounds::unbounded();
            for child in children {
                result = result.intersect(extract_date_bounds(child, date_fields));
            }
            result
        }
        FilterExpr::Or(children) => {
            ...
            let mut result = extract_date_bounds(&children[0], date_fields);
            for child in &children[1..] {
                result = result.union(extract_date_bounds(child, date_fields));
            }
            result
        }
        FilterExpr::Not(inner) => extract_date_bounds(inner, date_fields).invert(),
        FilterExpr::Leaf(leaf) => extract_leaf_bounds(leaf, date_fields),
        // A host predicate says nothing about a date range this crate can see.
        FilterExpr::Custom { .. } => DateBounds::unbounded(),
    }
}
...
mod tests {
    ...
    const DATE_FIELDS: &[&str] = &["started_at", "ended_at", "created_at"];
    use crate::expr::*;
    ...
    fn and_with_range() {
        ...
        let bounds = extract_date_bounds(&expr, DATE_FIELDS);
