...
pub enum Operator {
    ...
    In,
    NotIn,
    /// Tree query: find tasks that have the named task as an ancestor.
    /// `[has_ancestor, AcmeCorp]` → resolves to `path LIKE '%/<id>/%'` for each match.
    HasAncestor,
...
impl Operator {
    pub fn from_str(s: &str) -> Option<Self> {
        match s {
            ...
            "!=" | "<>" | "ne" => Some(Self::Ne),
            ">" | "gt" => Some(Self::Gt),
            ">=" | "ge" | "gte" => Some(Self::Gte),
            "<" | "lt" => Some(Self::Lt),
            "<=" | "le" | "lte" => Some(Self::Lte),
            "like" | "LIKE" => Some(Self::Like),
            "not_like" | "NOT LIKE" => Some(Self::NotLike),
            "has" | "HAS" => Some(Self::Has),
...
fn is_col_ref(s: &str) -> bool {
    if s.starts_with('.') {
        // .column — must be a simple identifier after the dot
        return s[1..].chars().all(|c| c.is_alphanumeric() || c == '_');
    }
...
impl<'de> Visitor<'de> for FilterExprVisitor {
    ...
    /// Array form: `[lhs, op, rhs]`, `[lhs, op]` for null-checks,
    /// or `[tree_op, value]` for tree operators (has_ancestor, in_tree).
    fn visit_seq<A: SeqAccess<'de>>(self, mut seq: A) -> Result<Self::Value, A::Error> {
        let first_raw: String = seq
            .next_element()?
            .ok_or_else(|| de::Error::invalid_length(0, &"2 or 3 elements"))?;

        // Check if the first element is a tree operator (2-element shorthand).
        if let Some(tree_op) = Operator::from_str(&first_raw) {
            if matches!(tree_op, Operator::HasAncestor | Operator::InTree) {
                let val: serde_yaml::Value = seq
                    .next_element()?
                    .ok_or_else(|| de::Error::invalid_length(1, &"2 elements for tree operator"))?;
                let rhs = yaml_value_to_rhs(&val).map_err(de::Error::custom)?;
        ...
        // Standard form: first element is lhs column.
        let lhs = parse_col_ref(&first_raw);
