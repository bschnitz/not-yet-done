...
use crate::repository::{ProjectRepository, TagRepository, TagStyle, TagStylePatch};
...
impl TagService for TagServiceImpl {
    async fn add_global(
        ...
        self.tag_repository.insert_global(name, style).await
    }
    ...
    async fn edit(
        ...
        validate_style_patch(&style)?;
        match parse_tag_id(&id)? {
            TagId::Global(uuid) => {
                let tag = self.tag_repository.update_global(uuid, name, style).await?;
...
fn validate_style(style: &TagStyle) -> Result<(), AppError> {
    validate_color_opt(&style.fg_color)?;
    validate_color_opt(&style.bg_color)?;
    Ok(())
}
