...
use not_yet_done_task_core::repository::{
    ...
    TagRepositoryImplParameters, TaskOp, TaskRepository, TaskRepositoryImpl,
    TaskRepositoryImplParameters, TrackingRepositoryImpl, TrackingRepositoryImplParameters,
...
fn compute_path_two_levels() {
    ...
    let parents = HashMap::from([(root, None), (child, Some(root))]);
    assert_eq!(
        compute_path(child, &parents),
        format!("/{}/{}/", sid(root), sid(child))
    );
}
...
fn compute_path_deep_chain() {
    ...
    let c = Uuid::new_v4();
    let d = Uuid::new_v4();
    let parents = HashMap::from([(a, None), (b, Some(a)), (c, Some(b)), (d, Some(c))]);
    assert_eq!(
...
async fn insert_child_task_has_parent_path() {
    ...
    let child = repo
        .insert("Child".into(), Some(root.id), None, None)
        .await
        .unwrap();
    assert_eq!(
        child.path.unwrap(),
        format!("/{}/{}/", sid(root.id), sid(child.id))
    );
}
...
async fn insert_grandchild_path() {
    ...
    let b = repo
        .insert("B".into(), Some(a.id), None, None)
        .await
        .unwrap();
    let c = repo
        .insert("C".into(), Some(b.id), None, None)
        .await
        .unwrap();
    assert_eq!(
        c.path.unwrap(),
        format!("/{}/{}/{}/", sid(a.id), sid(b.id), sid(c.id))
    );
}
...
async fn reparent_updates_self_and_children() {
    ...
    // Move B under D
    let b_updated = repo
        .update_task(b.id, None, None, None, Some(Some(d.id)), None)
        .await
        .unwrap();
    assert_eq!(
        b_updated.path.unwrap(),
        format!("/{}/{}/", sid(d.id), sid(b.id))
    );

    let c_updated = repo.find_by_id(c.id).await.unwrap();
    assert_eq!(
        c_updated.path.unwrap(),
        format!("/{}/{}/{}/", sid(d.id), sid(b.id), sid(c.id))
    );
}
...
async fn reparent_to_root() {
    ...
    repo.update_task(b.id, None, None, None, Some(None), None)
        .await
        .unwrap();
...
async fn batch_insert_builds_paths() {
    ...
    assert_eq!(
        a.path.unwrap(),
        format!("/{}/{}/", sid(root.id), sid(results[0].id))
    );
}
...
async fn batch_reparent_multiple_tasks() {
    ...
    let root = repo.insert("Root".into(), None, None, None).await.unwrap();
    let a = repo
        .insert("A".into(), Some(root.id), None, None)
        .await
        .unwrap();
    let b = repo
        .insert("B".into(), Some(a.id), None, None)
...
async fn batch_insert_then_reparent_under_new() {
    ...
    repo.apply_batch(vec![TaskOp::Update {
        id: child.id,
        description: None,
        status: None,
        priority: None,
...
async fn deep_tree_10_levels() {
    ...
    for i in 0..10 {
        let task = repo
            .insert(format!("Level {i}"), parent_id, None, None)
            .await
            .unwrap();
        ids.push(task.id);
...
async fn wide_tree_20_children() {
    ...
    for i in 0..20 {
        let child = repo
            .insert(format!("Child {i}"), Some(root.id), None, None)
            .await
            .unwrap();
        assert_eq!(
            child.path.unwrap(),
...
async fn rebuild_all_paths_fixes_missing() {
    ...
    db.execute_unprepared("UPDATE task SET path = NULL")
        .await
        .unwrap();
...
async fn path_like_finds_descendants() {
    ...
    let _b = repo
        ...
        .unwrap();
    let _c = repo
        .insert("C".into(), Some(root.id), None, None)
        .await
        .unwrap();
...
async fn path_like_has_ancestor() {
    ...
    let _b = repo
        ...
        .unwrap();
    let _c = repo
        .insert("C".into(), Some(a.id), None, None)
        .await
        .unwrap();
...
async fn complex_tree_restructure() {
    ...
    let acmecorp = repo
        .insert("AcmeCorp".into(), Some(work.id), None, None)
        .await
        .unwrap();
    let globex = repo
        .insert("Globex".into(), Some(acmecorp.id), None, None)
        .await
        .unwrap();
    let globex_tickets = repo
        .insert("Tickets".into(), Some(globex.id), None, None)
        .await
        .unwrap();
    let proj_101 = repo
        .insert("PROJ-101".into(), Some(globex_tickets.id), None, None)
        .await
        .unwrap();
    let proj_202 = repo
        .insert("PROJ-202".into(), Some(globex_tickets.id), None, None)
        .await
        .unwrap();
    let daily = repo
        .insert("Daily".into(), Some(globex.id), None, None)
        .await
        .unwrap();
    let initech = repo
        .insert("Initech".into(), Some(acmecorp.id), None, None)
        .await
        .unwrap();
    let initech_tickets = repo
        .insert("Tickets".into(), Some(initech.id), None, None)
        .await
        .unwrap();
    let ticket_42 = repo
        .insert("#42".into(), Some(initech_tickets.id), None, None)
        .await
        .unwrap();
    let _private = repo
        .insert("Private".into(), Some(work.id), None, None)
        .await
        .unwrap();
    ...
    assert_eq!(
        initech.path.unwrap(),
        format!("/{}/{}/", sid(work.id), sid(initech.id))
    );
...
async fn batch_delete_preserves_paths() {
    ...
    repo.apply_batch(vec![TaskOp::Delete { id: child.id }])
        .await
        .unwrap();
...
async fn include_ancestors_adds_parent_chain() {
    ...
    let _c = repo
        ...
        .unwrap();
    let _d = repo
        .insert("D".into(), Some(root.id), None, None)
        .await
        .unwrap();

    // Filter: only C matches (by description).
    use not_yet_done_task_core::filter::query_filter::QueryOptions;
    use not_yet_done_task_core::filter::{ColRef, FilterExpr, FilterLeaf, Literal, Operator, Rhs};
    ...
    let without = repo
        .find_filtered_with_options(&expr, &QueryOptions::default())
        .await
        .unwrap();
...
async fn include_ancestors_with_already_matching_parent() {
    ...
    let _a = repo
        ...
        .unwrap();
    let _b = repo
        .insert("B".into(), Some(_a.id), None, None)
        .await
        .unwrap();
