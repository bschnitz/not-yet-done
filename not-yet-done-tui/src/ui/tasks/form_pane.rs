    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        let block = Block::default()
            ...
            .title_bottom(Span::styled(
                format!(" {} close  [ctrl+r] reset ", kb.label(&TasksAction::FormClose)),
...
fn render_filter_form(area: Rect, buf: &mut Buffer, filter: &FilterState, t: &crate::ui::theme::Theme) {
    ...
    if placeholder_y + 1 < area.bottom() {
        ...
        Paragraph::new(sep).render(Rect { x, y: placeholder_y, width, height: 1 }, buf);
    }
}
...
fn render_text_field(
    ...
    let label_line = Line::from(vec![
        ...
        Span::styled(
            field.label(),
            Style::default().fg(label_fg).add_modifier(if focused { Modifier::BOLD } else { Modifier::empty() }),
        ),
        // Inline error
    ...
    if input_y < area.bottom() {
        ...
        let (display_text, text_style) = if value.is_empty() {
            ...
            let cursor_pos = if focused { Some(filter.cursor_pos) } else { None };
            render_text_with_cursor(buf, x + 2, input_y, width.saturating_sub(4), value, cursor_pos, t, focused);
        ...
        Paragraph::new(input_line).render(Rect { x, y: input_y, width, height: 1 }, buf);
...
fn render_text_with_cursor(
    ...
    let view_start = if let Some(pos) = cursor_pos {
        if pos >= max_w { pos + 1 - max_w } else { 0 }
    } else {
        0
    };
...
fn render_status_field(
    ...
    if opts_y < area.bottom() {
        ...
        for (label, active, idx) in &statuses {
            ...
            let check_fg = if is_checked { t.primary() } else { t.text_dim() };
            ...
            let label_style    = Style::default().fg(text_fg).bg(text_bg)
