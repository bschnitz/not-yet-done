...
//! this module partitions a sequence of items *vertically* into a single
//! level of groups and sums one or more aggregate columns per group, plus a
//! grand total.
//!
...
//! and grand totals, the footer toggle — lives here with its own tests.
...
pub struct GroupPlan {
    ...
    /// the order their headers are emitted (first appearance). One inner entry
...
/// Partition `keys` into a single level of groups (preserving first-appearance
/// order) and sum the aggregate columns.
///
/// - `keys[i]` is item `i`'s group identity *and* the group-header label.
///   Items with an equal key coalesce; group order follows first appearance,
///   so the caller controls ordering by pre-sorting its items.
...
///
/// Headers are emitted in group order, each immediately followed by its items;
/// each header's `group` field indexes [`GroupPlan::group_totals`], totalled
/// over that group's items. An empty `keys` yields an empty plan (and, if
/// `footer`, no footer row — there is nothing to total).
pub fn group(keys: &[String], values: &[&[Option<i64>]], footer: bool) -> GroupPlan {
    ...
    // First-appearance order of group labels + their members.
...
mod tests {
    ...
    fn footer_appends_grand_total_row() {
        ...
        let with = group(&k, &[vals], true);
        assert_eq!(with.rows.last(), Some(&PlanRow::GrandTotal));
        let without = group(&k, &[vals], false);
    ...
    fn multiple_aggregate_columns_sum_independently() {
        ...
        let plan = group(&k, &[dur, count], true);
