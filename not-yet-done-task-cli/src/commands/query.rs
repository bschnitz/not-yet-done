...
pub mod cli {
    ...
    /// dates, and executes the filter against the database. Output is the same
    /// flat JSON the saved-query store and adapters consume, which makes this a
    /// handy debugging tool for filter expressions before they go into a
    /// saved query.
    ///
    /// Examples:
    ///   nyd-t query run --entity tracking --file filter.yaml
    ///   echo 'query: [deleted, =, false]' | nyd-t query run --entity task
