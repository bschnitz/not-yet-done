...
impl Component for MultiChoiceWrapper {
...
impl AppComponent<Msg, NoUserEvent> for MultiChoiceWrapper {
...
impl AppComponent<Msg, NoUserEvent> for PlaylistNameComp {
    fn on(&mut self, ev: &Event<NoUserEvent>) -> Option<Msg> {
        if let Event::Keyboard(key_ev) = ev {
            match key_ev.code {
                Key::Tab => return Some(Msg::FocusNext),
            ...
            return match self.component.on(&Event::Keyboard(*key_ev)) {
                ...
                Some(TextInputEvent::CursorMoved(_)) => Some(Msg::Redraw),
                None => None,
            };
        }
        None
...
impl Model {
    ...
    fn validate(&mut self) -> bool {
        let name_ok =
            if let Ok(State::Single(StateValue::String(s))) = self.app.state(&Id::PlaylistName) {
                !s.is_empty()
            } else {
                false
            };
