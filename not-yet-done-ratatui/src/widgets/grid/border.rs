...
pub(super) enum NormalisedPos {
    ...
    VGap(usize),
    HGap(usize),
    VGapSpanned { gap_idx: usize, start: usize, end: usize },
    HGapSpanned { gap_idx: usize, start: usize, end: usize },
}
    ...
    fn normalise(&self, rows: usize, cols: usize) -> Option<NormalisedPos> {
        match *self {
            ...
            BorderPos::AfterCol(i) => {
                if i + 1 < cols { Some(NormalisedPos::VGap(i)) } else { None }
            }
            BorderPos::BeforeCol(i) => {
                if i > 0 && i < cols { Some(NormalisedPos::VGap(i - 1)) } else { None }
            }
            BorderPos::AfterRow(i) => {
                if i + 1 < rows { Some(NormalisedPos::HGap(i)) } else { None }
            }
            BorderPos::BeforeRow(i) => {
                if i > 0 && i < rows { Some(NormalisedPos::HGap(i - 1)) } else { None }
            }
            BorderPos::AfterColSpanned { col, row_start, row_end } => {
                if col + 1 < cols && row_start <= row_end && row_end < rows {
                    Some(NormalisedPos::VGapSpanned { gap_idx: col, start: row_start, end: row_end })
                } else { None }
            }
            BorderPos::BeforeColSpanned { col, row_start, row_end } => {
                if col > 0 && col < cols && row_start <= row_end && row_end < rows {
                    Some(NormalisedPos::VGapSpanned { gap_idx: col - 1, start: row_start, end: row_end })
                } else { None }
            }
            BorderPos::AfterRowSpanned { row, col_start, col_end } => {
                if row + 1 < rows && col_start <= col_end && col_end < cols {
                    Some(NormalisedPos::HGapSpanned { gap_idx: row, start: col_start, end: col_end })
                } else { None }
            }
            BorderPos::BeforeRowSpanned { row, col_start, col_end } => {
                if row > 0 && row < rows && col_start <= col_end && col_end < cols {
                    Some(NormalisedPos::HGapSpanned { gap_idx: row - 1, start: col_start, end: col_end })
                } else { None }
            }
        }
    }
